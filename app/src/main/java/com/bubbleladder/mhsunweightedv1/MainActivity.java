...
public class MainActivity extends Activity{
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리 · DUAL-RANGE FINAL1 V1.10",24,Color.WHITE,true));TextView sub=tv("1차 V1.7 + V1.9 합성 → 남은 3개 중 최종 1픽 하나만 추천",12,Color.rgb(110,231,183),false);sub.setPadding(0,dp(4),0,dp(14));root.addView(sub);
        ...
        LinearLayout hero=card();hero.addView(tv("다음 회차 · 최고 1픽",12,Color.GRAY,false));nextRound=tv("-",15,Color.WHITE,true);hero.addView(nextRound);bestPick=tv("분석 대기",30,Color.rgb(52,211,153),true);bestPick.setPadding(0,dp(7),0,0);hero.addView(bestPick);bestConf=tv("최종 1픽 단독검증률을 별도 표시",15,Color.rgb(253,224,71),true);bestConf.setPadding(0,dp(4),0,0);hero.addView(bestConf);TextView note=tv("1차 DUAL-RANGE로 1개 제외 · 2차에서 남은 3개 재분석 · 화면에는 최종 1픽 하나만 표시",12,Color.rgb(110,231,183),true);note.setPadding(0,dp(10),0,0);hero.addView(note);root.addView(hero);
        dimSummary=tv("",1,Color.TRANSPARENT,false);
        LinearLayout pc=card();pc.addView(section("2단계 최종 1픽 분석"));patternDetail=tv("-",13,Color.rgb(226,232,240),false);patternDetail.setLineSpacing(0,1.3f);pc.addView(patternDetail);root.addView(pc);
        LinearLayout bc=card();bc.addView(section("DUAL-RANGE 1차 + 2차 최종1픽 순차검증"));backtest=tv("-",14,Color.WHITE,false);backtest.setLineSpacing(0,1.3f);bc.addView(backtest);live=tv("실전 기록 · 아직 없음",14,Color.rgb(125,211,252),true);live.setPadding(0,dp(10),0,0);bc.addView(live);root.addView(bc);
        LinearLayout bet=card();bet.addView(section("최종 1픽 배팅 설정 · 수익 자동기록"));LinearLayout ir=new LinearLayout(this);ir.setOrientation(LinearLayout.HORIZONTAL);stake=input("5000");stake.setHint("배팅금액");stake.setInputType(InputType.TYPE_CLASS_NUMBER);odds=input("1.95");odds.setHint("배당");odds.setInputType(InputType.TYPE_CLASS_NUMBER|InputType.TYPE_NUMBER_FLAG_DECIMAL);ir.addView(stake,new LinearLayout.LayoutParams(0,dp(52),1));LinearLayout.LayoutParams olp=new LinearLayout.LayoutParams(0,dp(52),1);olp.setMargins(dp(8),0,0,0);ir.addView(odds,olp);bet.addView(ir);saveSetting=button("설정 저장 · 최소 5,000원",Color.rgb(30,64,175));LinearLayout.LayoutParams slp=new LinearLayout.LayoutParams(-1,dp(48));slp.setMargins(0,dp(8),0,0);bet.addView(saveSetting,slp);profit=tv("-",13,Color.rgb(226,232,240),false);profit.setPadding(0,dp(10),0,0);bet.addView(profit);root.addView(bet);
        ...
        root.addView(tv("※ 1차 DUAL-RANGE는 제외 필터 성적이고, 2차 최종 1픽 적중률은 별도입니다. 입력한 배당이 실제 최종 1픽 배당에 적용될 때만 수익 계산이 유효합니다.",11,Color.GRAY,false));return sv;
    }
    ...
    private void manualSync(){
        saveSettingsSilent();refresh.setEnabled(false);status.setText("API 수집 + V1.7/V1.9 합성 + 최종 1픽 계산 중...");
    ...
    private void render(FlowCore.Analysis a,List<FlowCore.Result>d){if(a==null||d==null||d.isEmpty())return;FlowCore.Result last=d.get(0);nextRound.setText(last.round<480?last.date+" · "+(last.round+1)+"회":"다음날 · 1회");
        ...
        FinalOnePickEngine.Result f=a.final1;
        if(f!=null&&f.pickCombo>=1&&f.pickCombo<=4){
            bestPick.setText(f.pickLabel);
            String first=(t!=null&&t.excludedCombo>=1&&t.excludedCombo<=4)?("1차 제외 "+t.excludedLabel):"1차 필터 준비";
            bestConf.setText("🎯 최종 1픽 · "+first+" · 2차 실검증 "+FlowCore.pct(f.validationRate)+(f.validationN>0?" · "+f.validationHit+"/"+f.validationN:" · 초기표본"));
            bestPick.setTextColor(f.validationRate>=0.513?Color.rgb(52,211,153):Color.rgb(253,224,71));
        }else{bestPick.setText("최종 1픽 분석 준비");bestConf.setText("데이터 동기화 필요");bestPick.setTextColor(Color.rgb(253,224,71));}
        ...
        if(t==null)ps.append("1차 DUAL-RANGE 준비중");else ps.append("[1차 V1.7 + V1.9 DUAL-RANGE]\n").append(t.detail).append("\n제외: ").append(t.excludedLabel).append("\n남은 3개: ").append(t.picksLabel);
        if(f!=null){ps.append("\n\n[2차 최종 1픽]\n").append(f.detail).append("\n최종: ").append(f.pickLabel).append("\n동등투표: ").append(f.votes).append("/").append(f.totalVotes).append("\n2차 경량 DUAL 순차검증: ").append(f.validationHit).append("/").append(f.validationN).append(" = ").append(FlowCore.pct(f.validationRate)).append("\n규칙: ").append(f.rule);}
        patternDetail.setText(ps.toString());
        FlowCore.Backtest b=a.backtest;
        backtest.setText("학습 데이터 "+a.count+"/150회 · "+a.windowRange+"\n현재 최근흐름: "+a.suffix+"\n\n[1차 DUAL-RANGE 필터]\n"+(t!=null&&t.excludedCombo>=1&&t.excludedCombo<=4?(t.validationN>0?t.validationHit+"/"+t.validationN+" = "+FlowCore.pct(t.validationRate):"초기표본"):"준비중")+"\n\n[2차 최종 1픽]\n"+(f!=null&&f.pickCombo>=1&&f.pickCombo<=4?(f.validationN>0?f.validationHit+"/"+f.validationN+" = "+FlowCore.pct(f.validationRate):"초기표본"):"준비중")+"\n\n[기초 MHS 참고]\n"+stat(b.globalHit,b.globalN)+"\n\n※ 각 검증 회차는 그 시점 이후 결과를 가리고 순차 재현");
        android.content.SharedPreferences sp=FlowCore.prefs(this);int n=sp.getInt(FlowCore.K_LIVE_TOTAL,0),hit=sp.getInt(FlowCore.K_LIVE_SUCCESS,0);double pnl=Double.longBitsToDouble(sp.getLong(FlowCore.K_LIVE_PROFIT,Double.doubleToLongBits(0)));live.setText("실전 최종1픽 · "+(n>0?hit+"/"+n+" = "+FlowCore.pct((double)hit/n):"아직 없음")+" · 누적손익 "+FlowCore.signed(pnl));int st=Math.max(5000,sp.getInt(FlowCore.K_BASE_STAKE,5000));double o=Math.max(1.01,sp.getFloat(FlowCore.K_ODDS,1.95f));double be=1.0/o;profit.setText("1회 배팅: "+FlowCore.money(st)+" · 배당 "+String.format(Locale.KOREA,"%.2f",o)+" · 손익분기 적중률 "+FlowCore.pct(be)+"\n적중 시 순이익 "+FlowCore.signed(st*(o-1.0))+" · 미적중 시 -"+FlowCore.money(st)+"\n※ 실제 최종 1픽에 이 배당이 적용되는 경우에만 손익 계산이 유효합니다.");
    ...
    private void startExport(){Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"BubbleDualRangeFinal1V110_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");startActivityForResult(i,REQ_EXPORT);}
